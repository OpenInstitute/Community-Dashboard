...
if($formname=="login") {
 ...
 $error = "";
     ...
     // $email = mysqli_real_escape_string($email);
      ...
      $qry = "SELECT roles.RoleUrl, roles.RoleId, public_login.name, public_login.id FROM public_login INNER JOIN roles ON public_login.RoleId = roles.RoleId WHERE public_login.email='$email' AND public_login.password='$password' LIMIT 1";
      //echo $qry; exit;
      ...
      $row = mysqli_num_rows($chk);
		...
		$user_id = $session['id'];
		mysqli_query($conn, "update `public_login` set lastLogin=CURRENT_DATE, `login_count` = (`login_count`+1) WHERE `id` = '$user_id'");
		
        $_SESSION['user_session'] = $session['name'];
        $_SESSION['RoleId'] = $session['RoleId'];
		
		$sess = array('us_id'=>''.$session['id'].'', 'us_name'=>''.$session['name'].'', 'us_role'=>''.$session['RoleId'].'');
		$_SESSION['sess_lanet_account'] = $sess;
		
		/* @Murage: Disable redirect to "public/" folder */
...
if($register == "register") {
      ...
      elseif(empty($organization)){
        ...
        header("location: ../register.php");}
          ...
          $q = "INSERT INTO `public_login`( `name`, `organization`, `email`, `password`, `avatar`,`RoleId`,`signedInFrom`,`lastLogin`) VALUES ('$name', '$organization', '$email', '$password', '','2','Web',now())";                            
