...
$sys_us_admin = array();	
	...
	//displayArray($_SESSION['sess_pom_member']);
if (!empty($_SESSION['sess_pom_member'])) 
{ 
	$sess_mbr	 = $_SESSION['sess_pom_member'];
	...
	$us_id		 = $sess_mbr['u_id'];	
	$us_name	   = $sess_mbr['u_name'];	
	$us_email 	  = $sess_mbr['u_email'];	
	$us_type_id  	= $sess_mbr['u_level_id'];	
	$us_level_id   = $sess_mbr['u_level_id'];	
	$us_level 	  = $sess_mbr['u_level'];
	...
	$us_acc_logout = '<a href="ca_posts.php?signout=on">Log Out <i class="fa fa-sign-out"></i></a>';
	
} else { $sess_mbr	= ''; }


if (!empty($_SESSION['sess_pom_admin'])) 
{ 
	$sys_us_admin = $_SESSION['sess_pom_admin'];
}
...
function conf_usAccLinks($acc_link_pos, $acc_roles='')
{
	...
	$us_staff = @$_SESSION['sess_pom_member']['u_staff'];
