...
class displays extends master
{
	...
	function getData($disp_query, $redirect, $disp_front = 0, $title_trunc = 80, $id_label = "id", $makeDatatable = "display") 
	{
		...
		$us_type_id  = $_SESSION['sess_pom_member']['u_level_id'];
		...
		<table border=0 cellpadding='0' cellspacing='0' width='100%' class='".$makeDatatable." table table-hover ' id='example'>
		<thead><tr>"; 
			if($disp_front ==10)
			{
			...
			for ($i = 0; $i<($this->num_fields); $i++)						// Field Names
			{
				...
				$field_names[$i] = $this->res_fields[$i]; 
				...
				for ($f=0 ; $f<=($this->num_fields-1); $f++) 
				{
					...
					$rc_email_lebo = "<a href='mailto:".$field."' title='Send email: ".$field."'>".$rc_email_text."</a>";
								...
								$field_names[$f]->name=="visibleX") 
						{
							...
							$lebo_id 	= $field_data[0]; //$field_data["id"];
							$lebo_field = $field;
