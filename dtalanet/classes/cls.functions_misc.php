...
class mailer 
{
	...
	function notifyPetitioner ($pt_num, $pt_email, $pt_stat, $pt_notes = '') 
	{
		$messageDetail = array("message_subject" =>"Petition '". $pt_num ."' Update",
							   "message_to"      =>"".$pt_email."",
							   "status"   	      =>"".$pt_stat."",
							   "pet_number"      =>"".$pt_num."",
							   "pet_notes"       =>"".$pt_notes."" );		
		$this->messageTemplate('notify_petitioner', $messageDetail, '');
	}
	...
	function messageTemplate($em_cat, $em_arr = array(), $cc_me = 1, $debug = 0) 
	{
		...
		//displayArray($em_arr); exit;
		
		if($em_cat <> '') {
						
			if($cc_me == 1) { $em_out['sendto'][] = "munene.murage@gmail.com"; }
			...
			{ 
				...
				case "notify_petitioner":
				
					//$em_out['sendto'][] = "".$em_arr['message_to']."";
					$em_out['subject']  = ''.SITE_TITLE_SHORT .' - '. $em_arr['message_subject'].'';
					
					$em_comments = ($em_arr['pet_notes'] <> '') ? '<p>'. $em_arr['pet_notes'] .'</p>' : '';
					
					$messageContent  = '<p>Hello,<br><br>Your '.SITE_TITLE_SHORT.' e-Petition number <strong>' . $em_arr['pet_number'] . '</strong> status has changed. </p>'
						.'<p>New status: <strong>' . $em_arr['status'] . '</strong></p>'
						.''. $em_comments .''
						.'<p><br><em>***<font size="2">This e-mail has been automatically generated. Do not reply to this address.</font></em></p>'
						.'<br><p><strong><u>'.SITE_TITLE_SHORT.' Webmaster</u></strong></p>'
						.'<br>';
...
<img src="'. SITE_DOMAIN_LIVE.'image/logo.png'.'" width="73"  alt="'. SITE_TITLE_SHORT.'"></a></td>
...
<td style="font-family:calibri,tahoma,arial,sans-serif;color:#666666;font-size:14px; padding:20px;padding-bottom:20px;">
