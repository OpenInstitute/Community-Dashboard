...
function dateAddDays($days = 1){
	$num_days = intval($days);
	$timestamp = time(); //-86400; //yesterday
	$date = strtotime("+".$num_days." day", $timestamp);
	return $date;
}
...
function generate_seo_title($str, $delimiter='-', $remove_words = true) {
	...
	if (extension_loaded('iconv')) {
		$clean = iconv("ISO-8859-15", "UTF-8", iconv("UTF-8", "ISO-8859-15//IGNORE", $clean));
	}
